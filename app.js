...
const DEFAULT_DB = {
  ...
  cities: {
    paris: { id: "paris", name: "Paris", country: "France", rating: 4.8, costIndex: 3, popularity: 9.8, img: "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?auto=format&fit=crop&w=600&q=80" },
    ...
    florence: { id: "florence", name: "Florence", country: "Italy", rating: 4.7, costIndex: 2, popularity: 9.2, img: "https://images.unsplash.com/photo-1541370976299-4d24ebbc9077?auto=format&fit=crop&w=600&q=80" },
    ...
    tokyo: { id: "tokyo", name: "Tokyo", country: "Japan", rating: 4.9, costIndex: 3, popularity: 9.9, img: "https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?auto=format&fit=crop&w=600&q=80" },
  ...
  trips: [
    {
      ...
      coverImg: "https://images.unsplash.com/photo-1499856871958-5b9627545d1a?auto=format&fit=crop&w=800&q=80",
      travelStyle: "Balanced",
      interests: ["Culture", "Food", "Adventure"],
      isPublic: true,
      stats: {
...
function renderShare(tripId) {
  ...
  document.getElementById("btn-copy-public-link").addEventListener("click", () => {
    const url = `${window.location.origin}${window.location.pathname}#share/${trip.id}`;
...
function renderAdmin() {
  ...
  const totalUsers = 1; // single user profile
  
  // Calculate average budget
  const avgBudget = Math.round(data.trips.reduce((acc, t) => acc + t.budget, 0) / totalTrips);
  
  mainContent.innerHTML = `
    <div>
      <h1 style="font-size:2.25rem; font-weight:800; margin-bottom:4px;">Admin Analytics</h1>
      <p style="color:var(--text-muted); margin-bottom:28px;">Monitoring platform engagements, database stops, and users.</p>
      ...
      <div class="budget-summary-grid">
        ...
        <div class="budget-widget-card">
          <p>Average Trip Budget</p>
          <h3>₹${avgBudget.toLocaleString()}</h3>
      ...
      <div class="upcoming-hero-card" style="padding:24px;">
        ...
        <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.95rem;">
          ...
          <tbody>
            ${Object.values(data.cities).map(city => {
              const stopCounts = data.trip_stops.filter(s => s.cityId === city.id).length;
              return `
                <tr style="border-bottom:1px solid var(--border-color);">
                  <td style="padding:12px 8px; font-weight:700;">${city.name}</td>
                  <td style="padding:12px 8px; color:var(--text-muted);">${city.country}</td>
                  <td style="padding:12px 8px; font-weight:600;"><i class="fa-solid fa-star" style="color:var(--accent);"></i> ${city.rating}</td>
                  <td style="padding:12px 8px; font-weight:700; color:var(--primary);">${stopCounts} stops</td>
                </tr>
              `;
            }).join("")}
          </tbody>
...
function renderLogin() {
    ...
    <div class="auth-page-container">
      <div class="auth-glass-card">
        <div class="auth-brand">
          ...
          <span class="logo-text">Globe<span>Trotter</span></span>
        </div>
        <h2 class="auth-title">Welcome Back</h2>
        <p class="auth-subtitle">Plan smarter. Explore further. Travel your way.</p>
        ...
        <form id="login-form" class="auth-form">
          <div class="auth-input-group">
            ...
            <div class="input-wrapper">
              ...
              <input type="email" id="login-email" placeholder="name@example.com" required value="sarah.j@globetrotter.io">
            </div>
          </div>
          <div class="auth-input-group">
            ...
            <div class="input-wrapper">
              ...
              <input type="password" id="login-password" placeholder="••••••••" required value="password">
            </div>
          </div>
  ...
  document.getElementById("login-form").addEventListener("submit", (e) => {
    ...
    const registeredUsers = JSON.parse(localStorage.getItem("globetrotter_auth_users") || '[{"email":"sarah.j@globetrotter.io","password":"password","name":"Sarah Jenkins"}]');
    ...
    if (matchedUser) {
      ...
      localStorage.setItem("globetrotter_current_user", JSON.stringify(matchedUser));
      
      // Update database user to match logged-in user
      const data = db.get();
      if (data && data.users) {
        data.users.name = matchedUser.name;
        data.users.email = matchedUser.email;
        db.save(data);
      }
      ...
      showToast("Invalid email or password. Hint: sarah.j@globetrotter.io / password", "error");
    }
  });
}
...
function renderRegister() {
  mainContent.innerHTML = `
    <div class="auth-page-container">
      <div class="auth-glass-card">
        ...
        <p class="auth-subtitle">Start planning your dream travel route today.</p>
  ...
  document.getElementById("register-form").addEventListener("submit", (e) => {
    ...
    localStorage.setItem("globetrotter_auth_users", JSON.stringify(registeredUsers));
...
/* ==========================================================================
   ...
   ========================================================================== */
const BUDGET_KEY = 'globetrotter_budget_analyzer';
...
function getBudgetData() {
  ...
  return {
    ...
    expenses: [
      { id: 'e1', desc: 'Hotel booking – Paris', category: 'accommodation', amount: 12000 },
      { id: 'e2', desc: 'Flight tickets',         category: 'transport',     amount: 8000 },
      { id: 'e3', desc: 'Restaurant meals',       category: 'food',          amount: 4500 },
      { id: 'e4', desc: 'Eiffel Tower tour',      category: 'activities',    amount: 2000 },
    ]
  };
}
